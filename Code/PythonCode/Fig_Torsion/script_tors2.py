...
def simulate(
    ...
    R1 = params.R1
    R2 = params.R2
    P0 = params.P0  # pitch/tilt
    P1 = params.P1
    P2 = params.P2
    BindMem0 = params.BindMem0
    BindMem1 = params.BindMem1
    BindMem2 = params.BindMem2
    BindFil0 = 0#- 0.04 - BindMem0 #0.0  # -0.015
    BindFil1 = 0#-1.25 - BindMem1  # -0.345
    BindFil2 = 0#-4.75- BindMem2  # -1.425
    kt_0 = k_0*params.ktFactor
    kt_1 = k_1*params.ktFactor
    kt_2 = k_2*params.ktFactor
    length=params.length
    #turns0 = length/(np.sqrt((2*np.pi*R0)**2+P0**2))
    ...
    Tau01 = (1 / (kt_0 + kt_1)) * (kt_0 * Tau0 + kt_1 * Tau1)
    Tau02 = (1 / (kt_0 + kt_2)) * (kt_0 * Tau0 + kt_2 * Tau2)
    Tau12 = (1 / (kt_1 + kt_2)) * (kt_1 * Tau1 + kt_2 * Tau2)
    Tau012 = (1 / (kt_0 + kt_1 + kt_2)) * (kt_0 * Tau0 + kt_1 * Tau1 + kt_2 * Tau2)
    ...
    def glob_no(curv, tau):
        if curv != 0:
            r = curv/(curv**2+tau**2)
            p = tau/(tau**2+curv**2)
            turns = length/(np.sqrt((2*np.pi*r)**2+p**2))
            pitch = p*turns
            g = 1/r
            gc = (8 / ((g * pitch) ** -2 + 1)) if (g and pitch) else 0
            cp = g * pitch
            if cp > 1:
                gc = cp - 1+4
        else:
    ...
    def bending_take(LocCurve,LocTor,site, c, Lattice, CurveSub, CurveNoSub,TorSub,TorNoSub, arg):
        ...
        for i in range(lattice_length):
            ...
            if i <= LimB and i >= LimS:
                ...
                LocTorAbs = LocTor[i]+ LocTorCalc
            else:
                LocCurveAbs = LocCurve[i]
                LocTorAbs = LocTor[i] 

            if Lattice[i] == 0:
                Ebend_takeaway += (1 / 2) * k_0 * (LocCurveAbs - Curve0) ** 2 + (1 / 2) * kt_0 * (LocTorAbs - Tau0) ** 2
            elif Lattice[i] == 1:
                Ebend_takeaway += (1 / 2) * k_0 * (LocCurveAbs - Curve0) ** 2 + (
                    1 / 2
                ) * k_1 * (LocCurveAbs - Curve1) ** 2 + (
                    1 / 2) * kt_0 * (LocTorAbs - Tau0) ** 2 + (
                    1 / 2) * kt_1 * (LocTorAbs - Tau1) ** 2
            elif Lattice[i] == 2:
                ...
                Ebend_takeaway += (1 / 2) * k_2 * (LocCurveAbs - Curve2) ** 2 + (
                    ...
                    1 / 2) * kt_2 * (LocTorAbs - Tau2) ** 2 + (
                    1 / 2) * kt_1 * (LocTorAbs - Tau1) ** 2
            elif Lattice[i] == 4:
                Ebend_takeaway += (1 / 2) * k_2 * (LocCurveAbs - Curve2) ** 2+(1 / 2) * kt_2 * (LocTorAbs - Tau2) ** 2
            elif Lattice[i] == 5:
                Ebend_takeaway += (1 / 2) * k_0 * (LocCurveAbs - Curve0) ** 2 + (
                    1 / 2
            ...
            if i == site:
                if arg == 1:
                    Ebend_takeaway = (
                        Ebend_takeaway - (1 / 2) * k_0 * (LocCurveAbs - CurveSub) ** 2 - (1 / 2) * kt_0 * (LocTorAbs - TorSub) ** 2
                    )
    ...
    for replica in range(replicas):
        ...
        for step in range(steps):
            ...
            for site in range(lattice_length):
                ...
                for n in range(*get_lims(site, N_neigh)):
                    ...
                    if lat_site_n < 7:
                        ...
                        LocTor[site] += label_2_loc_tors_delta[lat_site_n]
                ...
                if site == 0:
                    for site2 in range(lattice_length):
                        ...
                        if lat_site_n < 7:
                            g += 1
                            GlobCurve += label_2_loc_curve_delta[lat_site_n]
                            GlobTau += label_2_loc_tors_delta[lat_site_n]

                    GlobTau = GlobTau / lattice_length
                    if g != 0:
                        GlobCurve /= g
                # =============================================================================
                ...
                if Lattice[site] == 0:  # subunit 0 only
                    ...
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, 0,Tau0,0, 0)
                    
                    EbendNo0 = bending_take(LocCurve,LocTor,site, c, Lattice, Curve0, 0,Tau0,0, 1)
                    
                    Ebind0 = BindMem0
                    if GlobCurve == 0:
                        print("globCurve0 561")
                    Emem = glob_no(GlobCurve, GlobTau)

                    GlobCurve_no0 = GlobCurve - Curve0 / g
                    GlobTau_no0 = GlobTau - Tau0 / lattice_length
                    GlobCurve_ad1 = GlobCurve - Curve0 / g + Curve01 / g
                    GlobTau_ad1 = (
                        GlobTau - Tau0 / lattice_length + Tau01 / lattice_length
                    )
                    GlobCurve_ad2 = GlobCurve - Curve0 / g + Curve02 / g
                    GlobTau_ad2 = (
                        GlobTau - Tau0 / lattice_length + Tau02 / lattice_length
                    )

                    Emem_ad1 = glob_no(GlobCurve_ad1, GlobTau_ad1)

                    Emem_ad2 = glob_no(GlobCurve_ad2, GlobTau_ad2)

                    Emem_no0 = glob_no(GlobCurve_no0, GlobTau_no0)

                    Koff0 = (
                        np.exp(Ebind0)
                        * np.exp((EbendTot[step][replica] - EbendNo0))
                        * np.exp((Emem - Emem_no0))
                    )
                    ...
                    EbendNo0 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, Curve1,Tau0,Tau1, 1)
                    EbendNo1 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve1, Curve0,Tau1,Tau0, 1)
                
                    Ebind0 = BindFil0 + BindMem0
                    Ebind1 = BindFil1 + BindMem1
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, Curve1,Tau0,Tau1, 0)

                    Emem = glob_no(GlobCurve, GlobTau)

                    GlobCurve_no0 = GlobCurve + Curve1 / g - Curve01 / g
                    GlobTau_no0 = (
                        GlobTau + Tau1 / lattice_length - Tau01 / lattice_length
                    )
                    ...
                    GlobTau_ad2 = (
                        GlobTau - Tau01 / lattice_length + Tau012 / lattice_length
                    )

                    Emem_no0 = glob_no(GlobCurve_no0, GlobTau_no0)

                    Emem_no1 = glob_no(GlobCurve_no1, GlobTau_no1)
                    ...
                    Ebind1 = BindMem1
                    EbendNo1 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve1, 0,Tau1,0, 1)
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, Curve1,Tau0,Tau1, 0)
                    if GlobCurve == 0:
                        print("globCurve0 690")
                    Emem = glob_no(GlobCurve, GlobTau)

                    GlobCurve_no1 = GlobCurve - Curve1 / (g)
                    GlobTau_no1 = GlobTau - Tau1 / lattice_length
                    ...
                    GlobTau_ad2 = (
                        GlobTau - Tau1 / lattice_length + Tau12 / lattice_length
                    )

                    Emem_ad0 = glob_no(GlobCurve_ad0, GlobTau_ad0)
                    ...
                    EbendNo1 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve1, Curve2,Tau1,Tau2, 1)
                    EbendNo2 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, Curve1,Tau2,Tau1, 1)
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, Curve1,Tau2,Tau1, 0)
                    if GlobCurve == 0:
                        print("globCurve0 748")
                    Emem = glob_no(GlobCurve, GlobTau)
                    ...
                    GlobTau_no1 = (
                        GlobTau + Tau2 / lattice_length - Tau12 / lattice_length
                    )
                    GlobCurve_ad0 = GlobCurve - Curve12 / g + Curve012 / g
                    GlobTau_ad0 = (
                        GlobTau - Tau12 / lattice_length + Tau012 / lattice_length
                    )
                    ...
                    Emem_no2 = glob_no(GlobCurve_no2, GlobTau_no2)

                    Koff2 = (
                        np.exp((Ebind2))
                        * np.exp((EbendTot[step][replica] - EbendNo2))
                        * np.exp((Emem - Emem_no2))
                    )
                    Koff1 = (
                        np.exp((Ebind1))
                        * np.exp((EbendTot[step][replica] - EbendNo1))
                    ...
                    Ebind2 = BindMem2
                    if GlobCurve == 0:
                        print("globCurve0 812")
                    Emem = glob_no(GlobCurve, GlobTau)

                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve1, Curve2,Tau1,Tau2, 0)
                    EbendNo2 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, 0,Tau2,0, 1)

                    GlobCurve_no2 = GlobCurve - Curve2 / g
                    GlobTau_no2 = GlobTau - Tau2 / lattice_length
                    ...
                    GlobTau_ad0 = (
                        GlobTau - Tau2 / lattice_length + Tau02 / lattice_length
                    )
                    ...
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, Curve2,Tau0,Tau2, 0)
                    EbendNo0 = bending_take(LocCurve,LocTor,site, c, Lattice, Curve0, Curve2,Tau0,Tau2, 1)
                    EbendNo2 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, Curve0,Tau2,Tau0, 1)
                    if GlobCurve == 0:
                        print("globCurve0 872")
                    Emem = glob_no(GlobCurve, GlobTau)
                    ...
                    GlobTau_ad1 = (
                        GlobTau - Tau02 / lattice_length + Tau012 / lattice_length
                    )
                    ...
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, Curve01,Tau2,Tau01, 0)

                    EbendNo0 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve0, Curve12,Tau0,Tau12, 1)
                    EbendNo1 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve1, Curve02,Tau1,Tau02, 1)
                    EbendNo2 = bending_take(LocCurve,LocTor, site, c, Lattice, Curve2, Curve01,Tau2,Tau01, 1)
                    if GlobCurve == 0:
                        print("globCurve0 944")
                    # emem
                    Emem = glob_no(GlobCurve, GlobTau)
                    ...
                    EbendTot[step][replica] = bending_take(LocCurve,LocTor, site, c, Lattice, 0, 0,0,0, 0)
                    #if GlobCurve == 0:
                    #    print("globCurve0 1017")
                    Emem = glob_no(GlobCurve, GlobTau)

                    GlobCurve_ad0 = GlobCurve + Curve0 / (g + 1)
                    GlobTau_ad0 = GlobTau + Tau0 / lattice_length
                    GlobCurve_ad2 = GlobCurve + Curve2 / (g + 1)
                    GlobTau_ad2 = GlobTau + Tau2 / lattice_length
                    GlobCurve_ad1 = GlobCurve + Curve1 / (g + 1)
                    GlobTau_ad1 = GlobTau + Tau1 / lattice_length
            ...
            GlobPitchtot[step][replica] += GlobTau
