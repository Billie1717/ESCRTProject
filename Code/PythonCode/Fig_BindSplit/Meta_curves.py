...
#fname = str('bindsplit')
Dataloc = str('/storage/users/bmeadowcroft/Scratch/scripts/BindSplit/Data/')
fname = str('BindSplit2_')
...
for i in range(I):
    ...
    for j in range(J):
        ...
        #k_0 = (i+1)*0.2
        BindMem0 = Mem0*(j/19)
        BindMem1 = 3.2*Mem0*(j/19)
        BindMem2 = 11.5*Mem0*(j/19)
        #Curve1 = Curve0*(0.5*i)
        #Curve2 = Curve0*(0.5*j)
        #pitch1 = i*(0.1)
        #pitch2 = j*(0.1)
	#Curve1 = 1.0+np.sqrt((j))
        #Curve2 = Curve1 + np.sqrt(j*18)


        kwargs = dict(seed=np.random.randint(0,100),rand_array=None,replicas=200,lattice_length=42,t_max=t_max,
