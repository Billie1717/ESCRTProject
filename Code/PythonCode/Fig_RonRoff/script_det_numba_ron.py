...
def simulate(
    ...
    conc1 = 1.0
    conc2 = 1.0
    FKoff = params.FactorKoff
    FKa = params.FactorKa
    #FKa2 = FKa*0.5
    ka0 = conc0*FKa # association rate
    ka1 = conc1*FKa
    ka2 = conc2*FKa
    Koff0 = 0.8*FKoff   # Kon (constant through time)
    Koff1 = 0.8*FKoff 
    Koff2 = 0.8*FKoff 
    ...
    BindFil0 = 0.0 #-0.8 - BindMem0 #0.0  # -0.015
    BindFil1 = 0.0 #-0.8*3.2 - BindMem1  # -0.345
    BindFil2 = 0.0 #-0.8*11.5- BindMem2  # -1.425
    ...
    def glob_no(curv, pitch):
        ...
        cp = curv * pitch
        if cp > 1:
            gc = cp - 1 + 4 #was gc += cp - 1, corrected to gc = cp - 1 + 4
    ...
    def bending_take(LocCurve, site, c, Lattice, CurveSub, CurveNoSub, arg):
        ...
        if arg == 1:
            if c == 0:
                LocCurveCalc = 0
            elif arg == 2: #energy when subunit is added
        ...
        for step in range(steps):
            ...
            for site in range(lattice_length):
                ...
                if Lattice[site] == 0:  # subunit 0 only
                    ...
                    Ebendad1 = bending_take(LocCurve, site, c, Lattice, Curve01, Curve0, 2)
                    Ebendad2 = bending_take(LocCurve, site, c, Lattice, Curve02, Curve0, 2)
                    Ebind1 = BindMem1
                    Ebind2 = BindMem2
                    ...
                    Kon1 = (
                        np.exp(-(Ebind1))
                        * np.exp((EbendTot[step][replica] - Ebendad1))
                        * np.exp((Emem - Emem_ad1))
                    )
                    
                    Kon2 =(
                        np.exp(-(Ebind2))
                        * np.exp((EbendTot[step][replica] - Ebendad2))
                        * np.exp((Emem - Emem_ad2))
                    )
                    
                    BendEnergyDiff[step][replica] = EbendTot[step][replica] - EbendNo0

                    Ktot = ka1*Kon1 + ka2*Kon2 + Koff0
                    ...
                    Ebendad2 = bending_take(
                        LocCurve, site, c, Lattice, Curve012, Curve01, 2
                    )
                    ...
                    Ebendad0 = bending_take(LocCurve, site, c, Lattice, Curve01, Curve1, 2)
                    Ebendad2 = bending_take(LocCurve, site, c, Lattice, Curve12, Curve1, 2)
                    EbendTot[step][replica] = bending_take(
                    ...
                    Kon0 = (
                        np.exp(-(Ebind0))
                        * np.exp((EbendTot[step][replica] - Ebendad0))
                        * np.exp((Emem - Emem_ad0))
                    )
                    Kon2 = (
                        np.exp(-(Ebind2))
                        * np.exp((EbendTot[step][replica] - Ebendad2))
                    ...
                    Ebendad0 = bending_take(
                        LocCurve, site, c, Lattice, Curve012, Curve12, 2
                    )
                    EbendTot[step][replica] = bending_take(
                    ...
                    Ebendad0 = bending_take(LocCurve, site, c, Lattice, Curve02, Curve2, 2)
                    Ebendad1 = bending_take(LocCurve, site, c, Lattice, Curve12, Curve2, 2)
                    ...
                    Ebendad1 = bending_take(
                        LocCurve, site, c, Lattice, Curve012, Curve02, 2
                    )
                    EbendNo2 = bending_take(
                    ...
                    Ebendad0 = bending_take(
                        LocCurve, site, c, Lattice, Curve0, 0, 2
                    )
                    Ebendad1 = bending_take(
                        LocCurve, site, c, Lattice, Curve1, 0, 2
                    )
                    Ebendad2 = bending_take(
                        LocCurve, site, c, Lattice, Curve2, 0, 2
                    )
                    Emem = glob_no(GlobCurve, GlobPitch)
                    ...
                    Ktot = ka0*Kon0 + ka2*Kon2 + ka1*Kon1
                    tau = kot_r2tau(Ktot, r1)
                    Num_stay = tau / dt
                    if r3 < 1 / Num_stay:
                        if r2 < Kon0 / Ktot:
