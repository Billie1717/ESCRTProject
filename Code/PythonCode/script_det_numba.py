...
def simulate(
    ...
    ka1 = 0.8
    ka2 = 0.8
    Kon0 =(ka0 * conc0)**(np.log((params.FactorKa)))  # Kon (constant through time)
    Kon1 =(ka1 * conc1)**(np.log((params.FactorKa)))
    Kon2 =(ka2 * conc2)**(np.log((params.FactorKa)))
    ...
    # =============================================================================
    N_neigh = params.N_neigh  # neighbours to average local curvature
    # N = 42
        ...
        for step in range(steps):
            ...
            for site in range(lattice_length):
                ...
                if Lattice[site] == 0:  # subunit 0 only
                    ...
                    Koff0 = (
                        np.exp(Ebind0*np.log(FKoff))
                        * np.exp((EbendTot[step][replica] - EbendNo0)*np.log(FKoff))
                        * np.exp((Emem - Emem_no0)*np.log(FKoff))
                    )
                    ...
                    Koff1 = (
                        np.exp((Ebind1)*np.log(FKoff))
                        * np.exp((EbendTot[step][replica] - EbendNo1)*np.log(FKoff))
                        * np.exp((Emem - Emem_no1)*np.log(FKoff))
                    )
                    ...
                    Koff2 = (
                        np.exp((Ebind2)*np.log(FKoff))
                        * np.exp((EbendTot[step][replica] - EbendNo2)*np.log(FKoff))
                        * np.exp((Emem - Emem_no2)*np.log(FKoff))
                    )
                    Koff1 = (
                        np.exp((Ebind1)*np.log(FKoff))
                        * np.exp((EbendTot[step][replica] - EbendNo1)*np.log(FKoff))
